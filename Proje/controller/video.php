...
Class ControllerVideo Extends Controller {
	...
	public function index() {
		...
		$post 	= 	$this->model_post->getPost((int)$this->request->get('post_id'), (int)$this->config->get('video_parent_id'));
		...
		$this->data['post_share'] 						= social_share_urls($this->url->link('video', false, array('category' => $category->current['tag'], 'post_slug' => $post['slug'], 'post_id' => $post['id'])), $post['name']);
...
/*
		...
		if ( $images ) {
			...
			foreach ( $images as $image ) {
				...
				$this->data['post_images'][] = array(
					...
					'big' => $this->model_image->resize($image['image'], 627, 400)
				);
			}
			...
			$this->data['post_images'][] = array(
				'big' => $this->model_image->resize(false, 627, 400)
			);
		}*/
	    ...
	    if ( $query->rows ) {
		   	 ...
		   	 foreach ( $query->rows as $row ) {
		   		 ...
		   		 $data = array(
		   			 ...
		   			 'url'					=> $this->url->link('video', false, array('category' => $row['category'], 'post_slug' => $row['slug'], 'post_id' => $row['id']))
		...
		$this->template = 'video_detail.html';
		$this->children = array(
			'header',
			'menu',
			'footer'
