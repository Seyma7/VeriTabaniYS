...
Class ModelUpload extends Controller{
	...
	public function upload ( $file = false, $filename = false, $directory = false, $max_file_size = 300000, $allowed_types = array(), $watermark = false, $FWidth = 627, $FHeight = 268  ) {
		...
		if (!isset($data['error'])) {
			...
			if ($foo->uploaded) {
				...
				if($watermark){
					$foo->image_watermark    					= DIR_IMAGE.'watermark/watermark.png';
