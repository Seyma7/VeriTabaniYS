<?php
Class ControllerPostVideo Extends Controller{
	...
	public function index () {
		...
		$this->document->setTitle('Multimedya - Video');
		$this->data['description']	= "Video haber ekleyebilir, varolanları düzeltip silebilirsiniz. Yeni bir video haber eklemek için aşağıdaki <i>Ekle</i> butonunu tıklayınız.";
		...
		$sql 	= "
					...
					WHERE p.status NOT IN (6) && pc.category_id = '".$this->config->get('video_parent_id')."'
		...
		if ( $this->request->has('search_query') ) {
			...
			$sql .= " and LCASE(p.name) LIKE '%".$this->db->escape(mb_strtolower(urldecode($this->request->get('search_query')), 'UTF-8'))."%' ";
		}
		...
		$url = $this->url->link($this->request->get('controller'), false, array('sort' => $this->request->get('sort'), 'type' => $this->request->get('type'), 'page' => '{page}', 'search_query' 	=> $this->request->get('search_query')));
	...
	public function edit () {
		...
		$query = $this->db->query("
								...
								WHERE p.id = '".(int)$this->request->get('id')."' && pc.category_id = '".$this->config->get('video_parent_id')."'
		...
		if ($this->request->isPOST() && $this->validateForm()) {
			...
			$this->cache->delete('news.video');
			$this->session->set('success', $this->request->post('name') . ' isimli video başarıyla güncellendi.');
	...
	public function insert(){
		...
		$this->document->setTitle('Video Haber Ekle');
		...
		if ($this->request->isPOST() && $this->validateForm()) {
			...
			$this->cache->delete('news.video');
			$this->session->set('success', $this->request->post('name') . ' isimli video başarıyla eklendi.');
	...
	public function form () {
		...
		$category_list 									= array();
		$categories[] 									= $this->model_category->get_sub_categories(0, $this->config->get('video_parent_id'));

		$this->data['name'] 						= $this->db->escapeText($this->request->has('name', 'post') ? htmlspecialchars($this->request->post('name')) : (isset($this->post_info['name']) ? htmlspecialchars($this->post_info['name']) : ""));
		...
		$this->template = 'post_video_form.html';
		$this->children = array(
			'header',
			'footer'
		);
	...
	private function validateForm () {
		...
		if($this->request->post('video')){
			...
			/*if ( $video['name'] && strlen($video['name']) < 10 || strlen($video['name']) > 100 ) {
		...
		}else{$this->error = 'Video bilgilerini doldurmalısınız.';}
		...
		if (strlen($this->request->post('name')) < 3 || strlen($this->request->post('name')) > 100) {
			...
			$this->error = 'Haber başlığı 3 ile 100 karakter arasında olmalıdır.';
		}

		if ( !(int)$this->request->post('location') || !isset($location_list[(int)$this->request->post('location')]) ) {
